...
#摩耗なしのデータを読み込む
#データクラスによるクラス定義、初期値なのでイミュータブル(変更不可)として与える
@dataclasses.dataclass(frozen=True)
class NonWeardata_Group:
    testname="test"
    ...
    ARnW_offset["X"]=ARnW_offset["X"].add(-32.5)
    ARnW_offset["Y"]=ARnW_offset["Y"].add(-120.5)
    
    #摩耗なし断面の面積計算、ポリゴンデータ生成
...
class Setting_Group:
    ...
    #解析対象CSVフォルダのディレクトリをiniファイルによる指定
